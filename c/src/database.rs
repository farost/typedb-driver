...
pub extern "C" fn database_delete(database: *const Database) {
    unwrap_void(arc_from_raw(database).delete()); // TODO: Do we need to do anything else with the memory?
}

/// A full schema text as a valid TypeQL define query string.
#[no_mangle]
pub extern "C" fn database_schema(database: *const Database) -> *mut c_char {
    try_release_string(arc_from_raw(database).schema())
}

/// The types in the schema as a valid TypeQL define query string.
#[no_mangle]
pub extern "C" fn database_type_schema(database: *const Database) -> *mut c_char {
    try_release_string(arc_from_raw(database).type_schema())
}

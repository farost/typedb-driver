...
use typedb_driver::{Connection, DatabaseManager, Session, SessionType, Transaction, TransactionType};

const DB_NAME: &'static str = "benchmark";

fn prepare_session() -> Session {
...
fn prepare_transaction(session: &Session) -> Transaction<'_> {
    async_std::task::block_on(async {
        session.transaction(TransactionType::Read).await.expect("Expected transaction")
    })
}

fn rollback(transaction: &Transaction<'_>) {
    async_std::task::block_on(async {
...
fn multi_threaded_inserts() {
    ...
    let transaction: Arc<Transaction<'static>> = Arc::new(
    unsafe {
        std::mem::transmute(prepare_transaction(&session))
    });

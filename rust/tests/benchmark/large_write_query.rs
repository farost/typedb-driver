...
fn write_query(transaction: &Transaction, query: &str) {
    async_std::task::block_on(async {
        let mut rows: Vec<ConceptRow> = Vec::with_capacity(1000);
        let mut rows_stream = transaction.query(query).await.expect("Expected query result").into_rows();
        while let Some(row) = rows_stream.next().await {
            rows.push(row.expect("Expected row"));
        }
        let _ = rows[0].get_index(0).expect("Expected concept by index");
    })
}
